...
public:
    ...
    int findDist(Node* root, int a, int b) {
        // Find the paths from root to nodes a and b
        vector<int> pathA, pathB;
        if (!findPath(root, a, pathA) || !findPath(root, b, pathB))
            return -1; // One or both nodes not found
        
        // Traverse both paths to find the point of divergence
        int i = 0;
        while (i < pathA.size() && i < pathB.size() && pathA[i] == pathB[i])
            i++;
        
        // Minimum distance is sum of remaining lengths of both paths
